...
function PresentationCreator(props) {
  ...
  return (
    <Modal className="modal-dialog" isOpen={props.open} toggle={() => props.toggle(!props.open)}>
    <div className="modal-header">
      <h5 className="modal-title" id="exampleModalLabel">
        New Presentation
      </h5>
      <button aria-label="Close" className="close" data-dismiss="modal" type="button" onClick={() => props.toggle(!props.open)}>
    ...
    <div className="modal-body pt-0">
      <Form role="form" onSubmit={handleSubmit}>
        <FormGroup>
          ...
          <Input name="title" className="form-control-flush" id="presTitleInput" placeholder="e.g. Lecture 2A, Plant Roots Overview" type="text"/>
        </FormGroup>
        <FormGroup>
...
function mapStateToProps(state) {
  return {isCreating: state.presentations.isCreating}
}
